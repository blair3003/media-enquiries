...
export default function EnquiryTableRow({ enquiry: { id, subject, category, media, deadline } }) {
    return (
        <tr>
            <td className="py-4 px-3 pl-0 text-sm font-medium text-gray-900 max-w-prose truncate">
                <Link href={route('enquiry.show', id)} className="hover:underline">{subject}</Link>
            </td>
            ...
            <td className="py-4 px-3 pr-0 text-sm relative text-right font-medium">
                <Link href={route('enquiry.show', id)} className="text-indigo-600 hover:text-indigo-900 hover:underline">View</Link>
            </td>
        </tr>
    )
}
